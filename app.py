...
def render_landing_page():
    ...
    st.markdown(
        ...
        <div style="
            ...
            text-align: center;
            margin: 0 0 1.3rem 0;
            box-shadow: 0 8px 28px rgba(13,71,161,0.28);
        ">
            <div style="font-size:3.2rem; margin-bottom:0.55rem;">🏥</div>
            <h1 style="color:#fff; font-size:2.25rem; font-weight:900; margin:0; letter-spacing:-1px;">
                SADC
            </h1>
            <h2 style="color:#90caf9; font-size:1.05rem; font-weight:500; margin:0.4rem 0 0.65rem;">
                Sistema de Apoio à Decisão Clínica
            </h2>
            <p style="color:#bbdefb; font-size:0.95rem; margin:0;">
                Hospital Geral São José · Contagem/MG · Powered by Google Gemini
            </p>
        </div>
        """,
    ...
    cards = [
        (c1, "🤖", "#e8f5e9", "#2e7d32", "Avaliação Estruturada",
         "Gera documentação clínica (avaliação, encaminhamento e condutas) em minutos."),
        (c2, "⚗️", "#fff3e0", "#e65100", "Recursos do PA",
         "Adapta o raciocínio às limitações e ao que está disponível no HGS José."),
        (c3, "💾", "#e3f2fd", "#1565c0", "Histórico Local",
         "Prontuários salvos localmente para retomar atendimentos quando necessário."),
    ]
    for col, icon, bg, border, title, desc in cards:
        with col:
            st.markdown(
                f"""
                <div style="background:{bg}; padding:1.1rem 1rem; border-radius:12px;
                            border-left:4px solid {border}; min-height:150px;">
                    <div style="font-size:1.75rem; margin-bottom:0.35rem;">{icon}</div>
                    <h4 style="color:{border}; margin:0 0 0.35rem; font-size:0.95rem;">{title}</h4>
                    <p style="color:#4a4a4a; font-size:0.83rem; margin:0; line-height:1.45;">{desc}</p>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    st.markdown(
        ...
        <div style="background:#fff8e1; border:1px solid #ffe082;
                    border-left:5px solid #f9a825; padding:0.9rem 1.2rem;
                    border-radius:10px; margin: 1.2rem 0 1.6rem 0; font-size:0.9rem;">
            <strong>Limitações conhecidas (PA — HGS José)</strong><br>
            • Laboratório: Troponina, EAS (fita) e Gasometria.<br>
            • Imagem: ECG interpretado pelo médico; RX e TC sem contraste.<br>
            • Internação: ZERO leitos (foco em estabilizar + alta/transferência quando aplicável).
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        """
        <div style="text-align:center; color:#9e9e9e; margin-top:2.2rem; font-size:0.78rem;">
            SADC v1.0 · Hospital Geral São José — Contagem/MG<br>
            <em>Ferramenta de apoio à decisão clínica. Responsabilidade médica: exclusivamente do profissional habilitado.</em>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_sidebar():
    with st.sidebar:
        ...
        nav_choice = st.radio(
            "Navegação",
            options=["🩺 Atendimento", "📊 Scores clínicos"],
            index=0,
            label_visibility="collapsed",
            key="nav_main",
            horizontal=False,
        ...
        if nav_choice == "🩺 Atendimento":
            ...
            # Seleção rápida de score na barra lateral
            st.markdown("**📊 Scores clínicos**")
            score = st.selectbox(
                "Score",
                options=[
                    "qSOFA (sepse à beira-leito)",
...
"""
        ...
        with st.spinner("🤖 Processando avaliação clínica... Aguarde."):
            ...
            # Queixa principal (QP) curta para o #QP do "prontuário completo".
            # Como o formulário não tem um campo separado, derivamos a partir da primeira linha da HMA.
            hma_first_line = next(
                (ln.strip() for ln in hma.splitlines() if ln.strip()),
                "",
            )
            if ":" in hma_first_line:
                hma_first_line = hma_first_line.split(":", 1)[1].strip()
            queixa_principal = (hma_first_line or hma)[:120].strip()
            dados_extras["queixa"] = queixa_principal
...
# ════════════════════════════════════════════════════════════════
def _copy_area(content: str, height: int = 160, key: str | None = None):
    """Renderiza área de texto pronta para cópia."""
    st.caption("📋 Cópia rápida ↓  (clique · Ctrl+A · Ctrl+C)")
    if "_copy_area_counter" not in st.session_state:
        st.session_state["_copy_area_counter"] = 0
    if key is None:
        st.session_state["_copy_area_counter"] += 1
        key = f"copy_area_{st.session_state['_copy_area_counter']}"
    st.text_area(
        "",
        value=content,
        height=height,
...
def _split_section_for_ui(tag: str, content: str) -> tuple[str, str]:
    """
    Separa um bloco em:
    - topo: guia "o que procurar/checagem" (mais curto)
    - base: texto completo para cópia
    """
    ...
    if tag == "hma_box":
        marker = "seria importante complementar a história"
        idx = lower.find(marker)
        if idx != -1:
            guide = content[idx:].strip()
    ...
    if tag == "hpp_box":
        marker = "seria importante perguntar"
        idx = lower.find(marker)
        if idx != -1:
            guide = content[idx:].strip()
            body = content[:idx].strip()
    ...
    if tag == "exame_fisico_box":
        marker = "exame físico esperado para este quadro:"
        idx = lower.find(marker)
        if idx != -1:
            guide_block = content[idx:].strip()
            guide = re.sub(rf"(?i){re.escape(marker)}\s*", "", guide_block).strip()
            # base fica o bloco completo (realizado + esperado)
            return guide, content
        return "", content

    if tag == "hd_conduta_box":
        marker = "sinais de alerta"
        idx = lower.find(marker)
        if idx != -1:
            guide = content[idx:].strip()
            return guide, content
    ...
    if tag == "sit_box":
        # Guia mais curto para "fazer agora"
        marker = "exames externos solicitados"
        idx = lower.find(marker)
        if idx != -1:
            guide = content[:idx].strip()
            return guide, content
...
def _format_paragraphs_for_prontuario(text: str) -> list[str]:
    text = (text or "").strip()
    if not text:
        return []
    ...
    for p in raw_paras:
        ...
        if not p:
            continue
        # Mantém texto "limpo" sem quebras internas para não quebrar a marcação ^p
        p = " ".join(p.replace("\n", " ").split())
        paras.append(p)
    return paras


def _build_complete_prontuario_doc(parsed: dict, extras: dict) -> str:
    def _is_na(text: str) -> bool:
        ...
        if any(t.startswith(p) for p in na_prefixes):
            return True
        # Casos explícitos do próprio prompt
        if t.startswith("alta ou transferencia indicada") or t.startswith("alta ou transferência indicada"):
            return True
        if t.startswith("alta domiciliar indicada"):
            return True
        return False

    def section(header: str, text: str, default: str = "Não aplicável.") -> str:
        t = (text or "").strip()
        if not t:
            t = default
        # Quebras reais para colar no prontuário (uma seção por bloco).
        paras = re.split(r"\n\s*\n+", t.strip())
    ...
    qp = (extras or {}).get("queixa", "").strip() or "Não informado."

    parts: list[str] = [
        section("#QP", qp, default="Não informado."),
    ]

    if parsed and parsed.get("visao_geral"):
        parts.append(section("#VISAO_GERAL", parsed.get("visao_geral", "")))

    # Seções principais
    parts.append(section("#SITUACAO", parsed.get("sit_box", ""), default="Não aplicável."))
    parts.append(section("#HMA", parsed.get("hma_box", ""), default="Não aplicável."))
    parts.append(section("#HPP", parsed.get("hpp_box", ""), default="Não aplicável."))
    parts.append(section("#EXAME_FISICO", parsed.get("exame_fisico_box", ""), default="Não aplicável."))
    parts.append(section("#HD_CONDUTA", parsed.get("hd_conduta_box", ""), default="Não aplicável."))

    # Opcional/alto nível (copiar se estiver disponível)
    enc = (parsed or {}).get("encaminhamento_box", "")
    rx = (parsed or {}).get("receita_casa_box", "")
    ori = (parsed or {}).get("orientacoes_casa_box", "")
    ih = (parsed or {}).get("intra_hospitalar_box", "")

    # Só inclui encaminhamento se houver texto real (não apenas "Alta domiciliar indicada.")
    if not _is_na(enc):
        parts.append(section("#ENCAMINHAMENTO", enc, default="Não aplicável."))

    # Receita e orientações: só se forem aplicáveis (não "Nao aplicavel.")
    if not _is_na(rx):
        parts.append(section("#RECEITUARIO", rx, default="Não aplicável."))
    if not _is_na(ori):
        parts.append(section("#ORIENTACOES", ori, default="Não aplicável."))

    # Plano intra-hospitalar: só se for aplicável e não contradizer alta/transferência
    if not _is_na(ih):
        parts.append(section("#INTRA_HOSP", ih, default="Não aplicável."))

    return "\n\n".join(parts)


# ════════════════════════════════════════════════════════════════
# 11b. SCORES CLÍNICOS — FERRAMENTAS
# ════════════════════════════════════════════════════════════════


def _render_score_qsofa():
    st.markdown("### 📊 qSOFA — avaliação rápida de sepse")
    st.caption(
        "Uso à beira-leito para identificar pacientes com risco aumentado de mau desfecho em suspeita de infecção."
    )
    ...
    st.markdown(f"#### Resultado: **qSOFA = {score} pontos**")
    if score >= 2:
        st.error(
            "Alto risco de evolução desfavorável. Reavaliar imediatamente, procurar sinais de choque séptico e considerar transferência/UTI.",
            icon="⚠️",
        )
    elif score == 1:
        st.warning(
            "Risco intermediário — vigilância estreita, reavaliação seriada e investigação ativa de foco séptico.",
            icon="⚠️",
        )
    else:
        st.info(
            "Baixo risco pelo qSOFA isolado, mas o julgamento clínico e o lactato continuam sendo centrais.",
            icon="ℹ️",
        )


def _render_score_sofa():
    st.markdown("### 📊 SOFA simplificado — disfunção orgânica")
    st.caption(
        "Score formal de disfunção orgânica em sepse. Use preferencialmente em ambiente com gasometria e monitorização adequada."
    )

    with st.expander("Parâmetros (use o pior valor das últimas 24h)", expanded=True):
        col1, col2 = st.columns(2)
        with col1:
            pao2_fio2 = st.number_input("PaO₂/FiO₂ (mmHg)", min_value=0, max_value=600, value=400)
            plaquetas = st.number_input("Plaquetas (10³/mm³)", min_value=0, max_value=1000, value=200)
            bilirrubina = st.number_input("Bilirrubina total (mg/dL)", min_value=0.0, max_value=50.0, value=1.0)
        with col2:
            pam = st.number_input("PAM (mmHg)", min_value=0, max_value=200, value=70)
            vasopressor = st.selectbox(
                "Vasoativo em uso?",
                ["Nenhum", "Dopamina/ Dobutamina", "Noradrenalina/Adrenalina baixa", "Noradrenalina/Adrenalina alta"],
            )
            creat = st.number_input("Creatinina (mg/dL)", min_value=0.0, max_value=20.0, value=1.0)
            diurese = st.number_input("Diurese (mL/dia)", min_value=0, max_value=10000, value=1500)

        glasgow = st.slider("Glasgow", min_value=3, max_value=15, value=15)
    ...
    def cardio_points():
        if vasopressor == "Nenhum" and pam >= 70:
            return 0
        if vasopressor == "Nenhum" and pam < 70:
            return 1
        if vasopressor == "Dopamina/ Dobutamina":
            return 2
        if vasopressor == "Noradrenalina/Adrenalina baixa":
            return 3
        return 4
    ...
    def renal_points():
        if creat < 1.2 and diurese >= 500:
            return 0
        if creat < 2.0:
            return 1
        if creat < 3.5 or 200 <= diurese < 500:
            return 2
        if creat < 5.0 or diurese < 200:
            return 3
        return 4

    scores = {
        "Respiração": resp_points(),
        "Coagulação": coag_points(),
        "Fígado": hep_points(),
        "Cardiovascular": cardio_points(),
        "Neurológico": neuro_points(),
        "Renal": renal_points(),
    }
    total = sum(scores.values())

    st.markdown(f"#### Resultado: **SOFA = {total} pontos**")
    st.write(scores)
    if total >= 2:
        st.error("Disfunção orgânica significativa (SOFA ≥ 2) — compatível com definição de sepse.", icon="⚠️")
    else:
        st.info("SOFA baixo — sem disfunção orgânica relevante pelo score, mas mantenha vigilância clínica.", icon="ℹ️")


def _render_score_wells():
    st.markdown("### 📊 Score de Wells — TEP/TVP")
    st.caption(
        "Utilize em conjunto com o contexto clínico e a disponibilidade de dímero-D/angioTC, conforme descrito nos protocolos locais."
    )

    st.markdown("#### TEP")
    tep_items = {
        "Sinais clínicos de TVP (edema assimétrico, dor à palpação profunda)": 3.0,
        "TEP mais provável que diagnóstico alternativo": 3.0,
        "FC > 100 bpm": 1.5,
        "Imobilização > 3 dias ou cirurgia nas últimas 4 semanas": 1.5,
        "TVP ou TEP prévio documentado": 1.5,
        "Hemoptise": 1.0,
        "Neoplasia ativa": 1.0,
    }
    tep_score = 0.0
    for label, val in tep_items.items():
        if st.checkbox(label, key=f"wells_tep_{label}"):
            tep_score += val

    st.markdown(f"**Wells TEP = {tep_score:.1f} pontos**")
    if tep_score <= 4:
        st.info("Baixa/Moderada probabilidade (≤ 4) — em geral, indicar dímero-D POC se disponível.", icon="ℹ️")
    else:
        st.error("Alta probabilidade (> 4) — dímero-D pouco útil; considerar anticoagulação empírica e transferência.", icon="⚠️")

    st.markdown("---")
    st.markdown("#### TVP")
    tvp_items = {
        "Neoplasia ativa": 1,
        "Paresia/Imobilização de membro inferior": 1,
        "Acamado > 3 dias ou cirurgia major recente": 1,
        "Dor em trajeto venoso profundo": 1,
        "Edema de toda a perna": 1,
        "Edema de gémeos > 3 cm que a outra perna": 1,
        "Edema depressível unilateral": 1,
        "Veias superficiais colaterais visíveis": 1,
        "Diagnóstico alternativo tão ou mais provável": -2,
    }
    tvp_score = 0
    for label, val in tvp_items.items():
        if st.checkbox(label, key=f"wells_tvp_{label}"):
            tvp_score += val

    st.markdown(f"**Wells TVP = {tvp_score} pontos**")
    if tvp_score <= 1:
        st.info("Baixa probabilidade (≤ 1) — dímero-D para excluir TVP se negativo.", icon="ℹ️")
    else:
        st.error("Alta probabilidade (≥ 2) — considerar anticoagulação empírica e Doppler venoso.", icon="⚠️")


def _render_score_alvarado():
    st.markdown("### 📊 Alvarado — Apendicite Aguda")
    st.caption("Score clínico para estratificação de risco de apendicite, útil sobretudo em adultos.")

    items = [
        ("Dor migrando de região periumbilical para FID", 1),
        ("Anorexia", 1),
        ("Náuseas ou vômitos", 1),
        ("Dor à palpação em FID", 2),
        ("Rebote/Descompressão dolorosa (Blumberg) em FID", 1),
    ...
    for i, (label, pts) in enumerate(items):
        with cols[i % 2]:
            if st.checkbox(f"{label} (+{pts})", key=f"alvarado_{label}"):
                score += pts

    st.markdown(f"#### Resultado: **Alvarado = {score} pontos**")
    if score >= 7:
        st.error("Alto risco de apendicite (≥ 7) — forte indicação cirúrgica/avaliação imediata.", icon="⚠️")
    elif 5 <= score <= 6:
        st.warning("Risco intermediário (5–6) — investigação adicional / imagem e observação clínica.", icon="⚠️")
    else:
        st.info("Baixo risco (< 5) — considerar diagnósticos diferenciais, mantendo vigilância clínica.", icon="ℹ️")
...
def render_patient_view():
    ...
    with st.expander("📄 **1. Avaliação Inicial Completa**", expanded=True):
        ...
        for tab, tag in tab_data:
            with tab:
                ...
                guide, body = _split_section_for_ui(tag, content)
                if guide:
                    st.markdown(guide)
                    st.divider()
                _copy_area(body if body else content, height=130, key=f"copy_{tag}")
    ...
    with st.expander("🚑 **2. Encaminhamento / Transferência (Vaga Zero)**"):
        ...
        # Para evitar duplicação, mantenha o conteúdo principalmente no bloco de cópia.
        if is_na:
            st.markdown(content)
        if not is_na and content != "_Não gerado._":
            _copy_area(content, height=280, key="copy_encaminhamento")
    ...
    with st.expander("🏠 **3. Alta Domiciliar — Receituário & Orientações**"):
        ...
        with col_rx:
            ...
            if not is_na_rx and c_rx != "_Não gerado._":
                _copy_area(c_rx, height=180, key="copy_receita_casa")
        ...
        with col_ori:
            ...
            if not is_na_ori and c_ori != "_Não gerado._":
                _copy_area(c_ori, height=180, key="copy_orientacoes_casa")
    ...
    for i, msg in enumerate(st.session_state.chat_history):
        ...
        if role == "user":
            ...
            with st.chat_message("assistant", avatar="🤖"):
                # Não despeja novamente o "texto já mostrado nos cards":
                # o documento final único fica no bloco "Prontuário completo" ao final.
                if i == 1:
                    st.caption("_Avaliação inicial gerada — use os cards e o prontuário completo ao final_")
                else:
                    st.caption("_Prontuário atualizado — use o prontuário completo ao final_")

    # ════════════════════════════════════════════════════════
    # PRONTUÁRIO COMPLETO (documento único para copiar)
    # ════════════════════════════════════════════════════════
    st.markdown("### 🧾 Prontuário completo")
    complete_doc = _build_complete_prontuario_doc(parsed, extras)
    st.text_area("", value=complete_doc, height=360, label_visibility="collapsed", key="prontuario_completo")
    st.divider()
...
def render_scores():
    page = st.session_state.get("scores_page") or "qSOFA (sepse à beira-leito)"
    ...
    if "qSOFA" in page:
        ...
        _render_score_wells()
    elif "Alvarado" in page:
        _render_score_alvarado()
    else:
        st.info("Selecione um score na barra lateral para iniciar.")
...
def main():
    ...
    # Conteúdo principal
    nav = st.session_state.get("nav_main", "🩺 Atendimento")
    if nav == "📊 Scores clínicos":
        render_scores()
        return
